...
import '../Models/TimingModel.dart';
import '../main.dart';
import 'Custom/GlobalFunction.dart';

class EditTiming extends StatefulWidget{
  TimingModel timingModel;
  EditTiming({required this.timingModel});
  @override
  State<StatefulWidget> createState() {
    return _state(timingModel: this.timingModel);
  }
}
class _state extends State<EditTiming>{
  TimingModel timingModel;
  _state({required this.timingModel});
  var startTime;
  var endTime;
  ...
  loadData()async{
    ...
    setState(() {
      loading=false;
      event_action=timingModel.eventAction=="1"?true:false;
      device_action=timingModel.deviceAction=="1"?true:false;
      startTime=timingModel.startTime;
      endTime=timingModel.endTime;
      device_number.text=timingModel.userRoomDeviceId.toString();
    });
    for(int i=0;i<timingModel.days.length;i++){
      for(int j=0;j<days.length;j++){
        if(days[j].day==timingModel.days[i]){
          setState(() {
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child:  TimePickerSpinner(
                                  ...
                                  itemHeight: 30,
                                  isForce2Digits: true,
                                  time: DateTime.parse("2022-10-05 "+timingModel.startTime),
                                  onTimeChange: (time) {
                                    setState(() {
                                      startTime = time;
                                    });
                              ...
                              Container(
                                ...
                                child:  TimePickerSpinner(
                                  ...
                                  spacing: 35,
                                  itemHeight: 30,
                                  time: DateTime.parse("2022-10-05 "+timingModel.endTime),
                                  isForce2Digits: true,
                                  onTimeChange: (time) {
                                    setState(() {
                                      endTime = time;
                      ...
                      children: [
                        GestureDetector(
                          onTap: ()async{
                            ...
                            await deviceProvider.editTiming(timingModel.id,startTime.toString().substring(10,19), device_number.text, endTime.toString().substring(10,19), event_action?1:0, device_action?1:0, _days);
